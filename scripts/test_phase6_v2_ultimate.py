...
class PredictiveInterpolator:
    ...
    def __init__(self, ratio=25, lookahead=0.2):
        self.ratio = ratio
        self.lookahead = lookahead  # 前馈预测时间 (秒)
        self.q_start = None
        self.q_end = None
    ...
    def interpolate(self, step):
        """带前馈的插值: step in [0, ratio-1]"""
        # 前馈补偿：提前预测
        # lookahead=0.2秒意味着提前10步（0.2/0.002=100，但插值比例是25）
        lookahead_steps = min(int(self.lookahead * self.ratio / 0.05), self.ratio - 1)

        alpha = (step + lookahead_steps) / self.ratio
        alpha = min(alpha, 1.0)  # 限制在[0,1]

        return (1 - alpha) * self.q_start + alpha * self.q_end


class Phase6V2Ultimate:
    """Phase 6-v2 终极优化版本"""
    ...
    def __init__(self):
        ...
        self.interpolator = PredictiveInterpolator(
            ratio=self.interpolation_ratio,
            lookahead=0.15  # 提前0.15秒预测
        )
    ...
    def run(self):
        """运行控制循环"""
        print("启动 Phase 6-v2 双臂终极优化版本")
        print("=" * 60)
        print(f"优化策略组合:")
        print(f"  1. 固定基座IK（解决双臂耦合）")
        print(f"  2. 线性插值（平滑轨迹）")
        print(f"  3. 前馈预测（lookahead=0.15s）")
        print(f"  4. 适度降速（omega=0.3 rad/s）")
        print(f"")
        print(f"参数:")
        ...
        print(f"  轨迹周期: {2*np.pi/self.omega:.1f}s")
        print()
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            ...
            while viewer.is_running() and self.time < self.duration:
                ...
                # MPC更新 (20 Hz) - 使用前馈预测的目标
                if mpc_counter % self.interpolation_ratio == 0:
                    # 预测未来位置（前馈补偿）
                    t_future = self.time + self.interpolator.lookahead
                    target_left_future, target_right_future = self.circle_trajectory(t_future)

                    # 对未来位置求解IK
                    q_ik = self.ik_planner.solve_ik_fixed_base(target_left_future, target_right_future)
                    ...
                    # 验证IK精度（对当前目标，不是未来目标）
                    q_ik_current = self.ik_planner.solve_ik_fixed_base(target_left, target_right)
                    pos_left = self.pin_model.fk_left_ee(q_ik_current)
                    pos_right = self.pin_model.fk_right_ee(q_ik_current)
    ...
    def print_statistics(self):
        ...
        print(f"\n优化进展:")
        print(f"  原始版本 (omega=0.5, 无插值):    18.52 cm")
        print(f"  插值版本 (omega=0.5):           ~14.5 cm")
        print(f"  降速版本 (omega=0.2):            13.73 cm")
        print(f"  终极版本 (omega=0.3+前馈):       {avg_rms:.2f} cm")
        ...
        if avg_rms < 5.0:
            ...
            print(f"\n△ 需要进一步优化: {avg_rms:.2f} cm")


if __name__ == '__main__':
    controller = Phase6V2Ultimate()
    controller.run()
