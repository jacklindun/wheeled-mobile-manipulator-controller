...
"""
Phase 6-v2 双臂完整版本
架构: 固定基座IK (20Hz) → 插值器 (25:1) → 前馈PD (500Hz) → MuJoCo高增益
...
class Phase6V2Complete:
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            ...
            while viewer.is_running() and self.time < self.duration:
                ...
                if mpc_counter % self.interpolation_ratio == 0:
                    ...
                    # 设置插值器
                    self.interpolator.set_trajectory(
                        q_start=self.mj_data.qpos[:16].copy(),
                        q_end=q_ik,
                        v_start=self.mj_data.qvel[:16].copy(),
                        v_end=np.zeros(16)
                    )
    ...
    def print_statistics(self):
        ...
        print(f"\n对比:")
        print(f"  原始版本: 18.52 cm (IK残差 0.09 cm)")
        print(f"  当前版本: {avg_rms:.2f} cm (IK残差 {avg_ik:.3f} cm)")

        if avg_rms < 5.0:
            print(f"\n✓ 成功! 平均误差 {avg_rms:.2f} cm < 5 cm")
        elif avg_rms < 18.0:
            print(f"\n△ 改善: 从18cm降低到{avg_rms:.2f}cm")
        else:
            print(f"\n✗ 未改善: {avg_rms:.2f} cm")
