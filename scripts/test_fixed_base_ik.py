#!/usr/bin/env python3
"""
Phase 6-v2 双臂 - 固定基座IK版本
目标: 解决双臂IK耦合冲突

策略: 固定基座DOF，只优化两个手臂的6+6=12个关节
"""
...
class FixedBaseIKPlanner:
    ...
    def solve_ik_fixed_base(self, target_left, target_right, max_iter=100):
        ...
        for i in range(max_iter):
            ...
            # 堆叠成大系统: [error_left; error_right] = [J_left; J_right] * dq
...
class FixedBaseTest:
    ...
    def run(self):
        """运行控制循环"""
        print("启动 Phase 6-v2 固定基座双臂IK测试")
        print("=" * 60)
        print(f"策略: 固定基座，只优化12个手臂关节")
    ...
    def print_statistics(self):
        ...
        print(f"\n分析:")
        print(f"  IK精度: {avg_ik:.3f} cm {'✓' if avg_ik < 0.5 else '✗'}")
        print(f"  跟踪误差: {avg_rms:.2f} cm {'✓' if avg_rms < 5.0 else '✗'}")

        if avg_ik < 0.5 and avg_rms < 5.0:
            print(f"\n✓ 成功! IK精确且跟踪良好")
        elif avg_ik < 0.5:
            print(f"\n△ IK精确但跟踪差: 需要提高执行器增益或改进控制器")
        else:
            print(f"\n✗ IK失败: 需要改进IK算法")


if __name__ == '__main__':
    controller = FixedBaseTest()
    controller.run()
