...
class Phase6V2Slowdown:
    """Phase 6-v2 降速版本"""

    def __init__(self, omega=0.2):
        # 加载MuJoCo模型 - 高增益版本
        ...
        # 轨迹参数
        self.omega = omega  # 可调节的角速度
        self.duration = 20.0  # 增加时长以完成更多圈
        self.time = 0.0
    ...
    def run(self):
        """运行控制循环"""
        print("启动 Phase 6-v2 双臂降速优化版本")
        print("=" * 60)
        print(f"架构: 固定基座IK → 线性插值 → MuJoCo高增益")
        print(f"优化策略: 降低轨迹速度")
        print(f"角速度: omega={self.omega:.2f} rad/s (原始0.5 rad/s)")
        print(f"周期: {2*np.pi/self.omega:.1f}s")
    ...
    def print_statistics(self):
        ...
        print(f"\n对比:")
        print(f"  原始版本 (omega=0.5): ~18 cm")
        print(f"  插值版本 (omega=0.5): ~14.5 cm")
        print(f"  降速版本 (omega={self.omega:.2f}): {avg_rms:.2f} cm")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--omega', type=float, default=0.2,
                       help='轨迹角速度 (rad/s), 默认0.2')
    args = parser.parse_args()

    controller = Phase6V2Slowdown(omega=args.omega)
    controller.run()
