...
基于第一轮结果，围绕High_PD_Gains配置进行精细调优
"""
...
print("="*80)
print("Phase 6-v2 精细调优 - 第二轮")
print("="*80)
print("目标: RMS误差 < 2.5 cm")
print("="*80)

# 第二轮测试配置 - 围绕High_PD_Gains微调
TEST_CONFIGS = [
    # Config 1: High_PD_Gains (baseline from round 1)
    {
        "name": "Round1_Best",
        "mpc_weights": None,
        "pd_gains": {
    ...
    # Config 3: Higher MPC weights + High PD
    {
        "name": "High_MPC_Weight",
        "mpc_weights": {
            "ee_pos": 200.0,
            "terminal_ee_pos": 500.0,
            "base_xy": 80.0,
            "base_z": 80.0,
        },
        "pd_gains": {
    ...
    # Config 4: Longer horizon + High PD
    {
        "name": "Long_Horizon_High_PD",
        "mpc_weights": None,
        "pd_gains": {
            "Kp_base_xy": 100.0, "Kd_base_xy": 20.0,
            "Kp_base_z": 1000.0, "Kd_base_z": 200.0,
    ...
    # Config 6: Ultra-tuned (combined best guesses)
    {
        "name": "Ultra_Tuned",
        "mpc_weights": {
            "ee_pos": 300.0,
        ...
        "pd_gains": {
            "Kp_base_xy": 120.0, "Kd_base_xy": 25.0,
            "Kp_base_z": 1200.0, "Kd_base_z": 250.0,
            "Kp_arm": 1200.0, "Kd_arm": 120.0
        },
        "horizon": 18,
        "max_iters": 15,
    },
...
for i, config in enumerate(TEST_CONFIGS):
    ...
    try:
        ...
        if result['convergence_rate'] >= 98:
            score += 2
        elif result['convergence_rate'] >= 95:
            score += 1
...
print(f"\n{'配置':<25} | {'RMS(cm)':<10} | {'最大(cm)':<10} | {'收敛率':<10} | {'目标':<8} | {'评分'}")
print("-" * 95)
...
for item in results:
    ...
    if result:
        ...
        score_str = '⭐' * score
        print(f"{config['name']:<25} | {result['rms_error']:>8.2f}   | {result['max_error']:>8.2f}   | {result['convergence_rate']:>7.1f}%  | {goal_met:<6} | {score_str}")
    else:
        print(f"{config['name']:<25} | {'FAILED':<10} | {'FAILED':<10} | {'FAILED':<10} | {'❌':<6} | ")
...
if best:
    ...
    if best['result']['rms_error'] <= 2.5:
        ...
        print(f"\n⚠️  未达目标（{best['result']['rms_error']:.2f} cm > 2.5 cm），建议进一步调优")
    ...
    if best['config']['mpc_weights']:
        ...
        print(f"  mpc_weights = None  # Use defaults")
    print(f"  pd_gains = {{")
